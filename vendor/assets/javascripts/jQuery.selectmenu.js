...
$.fn.selectmenu = function(settings){
	...
	return $(this).each(function(){
		...
		var div=$('<div class="custom-select-wrapper"></div>').addClass(klass).css({
            margin:0
        });
        button.appendTo(div)
        div.insertAfter(selectElement);
		//menu icon
		...
		selectElement.find('option')
			...
			.each(function(index){
				...
				var li = $('<li class="'+ $(this).attr('class') +'"><a href="#" tabindex="-1" role="option" aria-selected="false">'+ '<span class="ui-text">'+o.format($(this).text())+'</span></a></li>');
